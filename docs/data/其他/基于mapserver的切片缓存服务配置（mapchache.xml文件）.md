---
title: 基于mapserver的切片缓存服务配置（mapchache.xml文件）
...
**mapcache 配置文件是一个 xml 文件，通过配置该文件来确定切片服务的类型以及特点，该文件在`mapserver\ms4w\apps\mapcache`目录下，包含以下配置项：**
...
**`source`包含了 mapcache 切片可以查询并获取到的原始图像数据或服务，通常是一个可以用 url 访问到的 wms 服务。具体配置如下：**
...
**cache 是切片后存放的位置，可以是文件夹、数据库，或者在线缓存等。具体配置如下：**
...
`<extent>`：要切片的地理范围，必须指定四个数值，用空格隔开，顺序依次为 minX，minY，maxX，maxY
...
`<srs>`：坐标系，通常为 EPSG 代号。仅用在接收 WMS 服务查找切片时。
...
`<resolutions>`：切片后每个缩放级别的分辨率。必须以正浮点值的形式表示，用空格分隔，按从大到小的顺序排列，最大值对应缩放级别 0。
...
以上就是 mapcache 发布切片时常用的配置项，其他配置项可去官网查看。
